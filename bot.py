...
async def EncRypTMajoRLoGin(open_id, access_token):
    ...
    major_login.client_version = "2.121.1"
            ...
            while True:
                ...
                if data.hex().startswith("120000"):
                    ...
                    if response:
                        ...
                        if inPuTMsG in ("13513476553" , "bot" , "Promotion"):
                            uid = response.Data.uid
                            chat_id = response.Data.Chat_ID
                            message = '\n[b][c][9999ff]Hi Makkaley,\nNaa Oru Robotuhh\n\nEna Uruvakunadhu [99ff99]\nᏴᴀᴛϻꫝͷㅤ❗❗❗\n\n[9999ff]Avanoda Insta [ff9999]krix_i43[9999ff]\n\nVera Edhachu Doubt Illa Kekanumna Avan Kittaye Kettukonga prnds.....[ff0000]❤️\n'
                            P = await SEndMsG(response.Data.chat_type , message , uid , chat_id , key , iv)
                            await SEndPacKeT(whisper_writer , online_writer , 'ChaT' , P)
                        response = None
...
async def perform_emote(team_code, uids, emote_id):
    ...
    await asyncio.sleep(0.5)
    Entry = await Emote_k(Bot_id, Heart, key, iv, region)
    await SEndPacKeT(None, online_writer, 'OnLine', Entry)
